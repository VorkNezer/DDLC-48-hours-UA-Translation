        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("Е”Е—Г±Д®ВјВ»Е§ГѕЕЂГ‚Е»Е•Д›ЕЌГ¬В«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Будь ласка введіть своє ім'я", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("Нова гра") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Введите имя героя", ok_action=Function(FinishEnterName)))

            else:

                textbutton _("Історія") action [ShowMenu("history"), SensitiveIf(renpy.get_screen("history") == None)]

                textbutton _("Зберегти") action [ShowMenu("save"), SensitiveIf(renpy.get_screen("save") == None)]

            textbutton _("Завантажити") action [ShowMenu("load"), SensitiveIf(renpy.get_screen("load") == None)]
...
init -1 style navigation_button_text:
    ...
    color "#fff"
    outlines [(0, "fff", 0, 0), (0, "fff", 0, 0)]
    hover_outlines [(2, "#dadada", 0, 0), (1, "#dadada", 1, 1)]
    insensitive_outlines [(2, "#fce", 0, 0), (1, "#fce", 1, 1)]
...
init -501 screen game_menu_h(title, scroll=None):
...
init -1 style game_menu_content_frame_h:
    left_margin 40
    right_margin 20
    top_margin 0
    bottom_margin 80
...
init -501 screen about() tag menu:
    ...
    use game_menu(_("Про гру")):

        style_prefix "about"
        vbox:
            xoffset -84
            label "{i}Ця модифікація була зроблена після двох діб плутанини в думках...{/i}" xalign 0.5
            text _("Люсі Нортон") xalign 0.5
            text _("") xalign 0.5
            text _("{i}Використана музика...{/i}") xalign 0.5
            text _("Nine Inch Nails") xalign 0.5
            text _("") xalign 0.5
            text _("{i}Беґраунди...{/i}") xalign 0.5
            text _("Kimagure After") xalign 0.5
            text _("Uncle Mugen") xalign 0.5
            text _("") xalign 0.5
            text _("{i}При підтримці...{/i}") xalign 0.5
            text _("Wretched Team") xalign 0.5
            text _("") xalign 0.5
            text _("{i}І Mood Posing Tool by...{/i}") xalign 0.5
            text _("Chronoshag & yagamirai10") xalign 0.5
...
init -1 python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="...",
                ok_action=Show(screen="dialog", message="...", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="...", ok_action=Hide("dialog"))
        else:
            ...
            hbox:
                ...
                if config.developer:
                    vbox:
                        style_prefix "radio"
                        label _("Відкат")
                        textbutton _("Без відкату") action Preference("rollback side", "disable")
                        textbutton _("Вліво") action Preference("rollback side", "left")
...
init -501 screen history() tag menu:
    ...
    predict False
    use game_menu_h(_("Історія"), scroll=("vpgrid" if gui.history_height else "viewport")):
...
init -501 screen name_input(message, ok_action):
    ...
    frame:
        ...
        input default "" value VariableInputValue("player") length 12 allow "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzАБВГҐДЕЄЖЗИІЇЙКЛМНОПРСТУФХЦЧШЩЬЮЯабвгґдеєжзиіїйклмнопрстуфхцчшщьюя"
